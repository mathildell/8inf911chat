...
function outputAllMessages(socket, collection) {
    collection.find({ chatroom : chatroomNbr }).limit(50).sort({date: 1}).toArray(function (err, res) {
        if (err) {
        ...
        socket.on('storage', function (storage_data) {
            ...
            for (var x = 0; x < res.length; x += 1) {
                ...
                var output = {
                    ...
                    avatar: res[x].avatar,
                    chatroom: res[x].chatroom
                };
                if(chatroomNbr !== output.chatroom){
                    output.originalChatroom = chatroomNbr;
                }
                //console.log(output);
...
function userInput(io,socket,collection,WhisperChek) {
    socket.on('userinput', function (data) {
        ...
        if (whitespacePattern.test(name) || whitespacePattern.test(message)) {
            ...
            collection.insert({date: date, name: name, message: message, avatar: avatar, chatroom: chatroom}, function () {
